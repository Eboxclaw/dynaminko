...
| ------------ | ------------------------------- | ------- | ------------------------------------------------------ |
| Manual       | User-initiated, any time        | Highest | Captures the _why_ before an outcome exists to bias it |
...
| Do                                                                                                                                                                           | Don't                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| Onyx `#0A0A0C` background, Obsidian `#151318` panels, Hairline `#2A2830` 1px borders only, Lavender `#B6A5F0` as the _single_ accent                                         | Default to shadcn's rounded cards + drop shadows anywhere. No drop shadows in this app, full stop                       |
...
| Mirror Nado's own confirmation discipline (gather context → present summary → ask → execute) for anything that touches money `[Nado MCP/CLI docs, §4.6]`          | Treat a user's _initial_ request as confirmation — Nado's own docs are explicit that it isn't, and Dynaminko should hold the same line |
...
export const PerformanceAxesSchema = z.object({
  ...
  financial: z
    .object({
      sizePctOfPortfolio: z.number().nullable(),
      leverage: z.number().nullable(),
    })
    .nullable(),
  psychological: z
    .object({
      entryTag: z.string().nullable(),
      exitTag: z.string().nullable(),
    })
    .nullable(),
});
